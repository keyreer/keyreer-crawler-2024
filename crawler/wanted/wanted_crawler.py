...
class Scraper:
    ...
    async def fetch_job_url_with_async(self, session: aiohttp.ClientSession) -> List:
        ...
        """
        print(
            f">>> Started collecting job listings for the position of {self.category_name}."
        )
        ...
        print(
            f">>> Completed collecting job listings for the position of a {self.category_name}."
        )
    ...
    async def fetch_job_description(
        ...
        if job_detail:
            ...
            latitude = (
                coordinate.get("location", dict()).get("lat", None)
                if coordinate is not None
                else None
            )
            longitude = (
                coordinate.get("location", dict()).get("lng", None)
                if coordinate is not None
                else None
            )
            ...
            try:
                job_detail_info = {
                    ...
                    "primary_responsibility": job_detail.get("detail", dict()).get(
                        "main_tasks", None
                    ),
                    "required": job_detail.get("detail", dict()).get(
                        "requirements", None
                    ),
                    "preferred": job_detail.get("detail", dict()).get(
                        "preferred_points", None
                    ),
                    "end_at": job_detail.get("due_time", None),
                    "skills": skills,
                    "location": job_detail.get("address", dict()).get(
                        "full_location", None
                    ),
                    "welfare": job_detail.get("detail", dict()).get("benefits", None),
                    "body": "",
                    "company_description": job_detail.get("detail", dict()).get(
                        "intro", None
                    ),
    ...
    async def run(self) -> List[Dict]:
        ...
        """
        print(
            f">>> Collecting job descriptions for the position of a {self.category_name}."
        )
        ...
        print(
            f">>> Completed collecting job descriptions for the position of a {self.category_name}."
        )
...
def main() -> None:
    ...
    for category_id, category_name in category.items():
        scraper = Scraper(
            base_url=base_url, category_id=category_id, category_name=category_name
        )
