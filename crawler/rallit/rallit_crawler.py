...
class Scraper:
    ...
    async def get_job_parser(
        self, p_index: int, session: aiohttp.ClientSession
    ) -> None:
            ...
            try:
                ...
                for row in rows:
                    ...
                    if a_tag:
                        ...
                        position_key_match = re.search(
                            r"/positions/(\d+)", position_url
                        )
                        if position_key_match:
                            position_key = position_key_match.group(1)
                            position_dict = await self.get_position_parser(
                                position_key=position_key, session=session
                            )
    ...
    async def get_position_parser(
        self, position_key: str, session: aiohttp.ClientSession
        ...
        async with session.get(position_url, headers=headers) as response:
            ...
            position_dict = {
                ...
                "coordinate": [
                    data_detail.get("latitude"),
                    data_detail.get("longitude"),
                ]
                if data_detail.get("latitude") and data_detail.get("longitude")
                else None,
            }
...
def main() -> None:
    ...
    for job_category in job_categories:
        ...
        scraped_data: List[Dict[str, Any]] = asyncio.run(
            scraper.get_object_thread(start=1, end=30)
        )
