...
class Scraper:
    ...
    async def get_job_lists(self, session, base_url: str) -> List[int]:
        ...
        for page in range(1, total_page_cnt + 1):
            ...
            job_id_lists.extend(
                [recruit["id"] for recruit in result["data"]["recruits"]]
            )
    ...
    async def get_job_descriptions(self, session, base_url: str, job_id: int) -> None:
        ...
        """
        result = await self.fetch_data_from_api(
            session, f"{base_url}api/v1/job/postings/{job_id}"
        )
        ...
        val = {
            ...
            "preferred": None
            if result["data"]["preferred_skill"] == ""
            else result["data"]["preferred_skill"].replace("\r\n", "\n"),
            ...
            "welfare": None
            if result["data"]["benefit"] == ""
            else result["data"]["benefit"].replace("\r\n", "\n"),
            "body": None,
            "company_description": None
            if result["data"]["introduction"] == ""
            else result["data"]["introduction"].replace("\r\n", "\n"),
            "coordinate": None,
        }
    ...
    async def main(self) -> List[Dict[str, Any]]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            tasks = [
                self.get_job_descriptions(session, self.base_url, job_id)
                for job_id in job_id_lists
            ]
